...
var chefAddress = "0xf2801E3C60585E126FD89568E9d5BDB249b20CcD"; // chef
var tokenAddress = "0xa9af9CB36d7FCBB21149628BDF76Cc8Aa8987FA5"; // kimbap token
...
var pools = [
    ["0xf967a70804891466ae3498528aa13f331f62fa3c", "UNISWAP KIMBAP/ETH", "https://uniswap.info/pair/0xf967a70804891466ae3498528aa13f331f62fa3c", 3, 0, 0],
    ["0x7b6ed7732a4f54824e6d99abf4a5b8fd7114e635", "UNISWAP KIMBAP/USDT", "https://uniswap.info/pair/0x7b6ed7732a4f54824e6d99abf4a5b8fd7114e635", 3, 0, 0],
    ["0xa404789a323779541d70234bee0f1cabc6437768", "UNISWAP KIMBAP/USDC", "https://uniswap.info/pair/0xa404789a323779541d70234bee0f1cabc6437768", 3, 0, 0],
    ["", "VOID", "", 0, 0, 0], //USDT/ETH
    ...
    ["0x0d4a11d5eeaac28ec3f61d100daf4d40471f1852", "UNISWAP USDT/ETH", "https://uniswap.info/pair/0x0d4a11d5eeaac28ec3f61d100daf4d40471f1852", 1, 0, 0]
];

var loadedpools = 0;
var totalPoolWeight = 12; // sum of weight
...
var oldTokenAddress = "0x7701Ba3946562f00b78dc789923A567A8238C108";
var swapContractAddress = "0x25cD53e8ddC1F31431531735A7a487E226a51556";
var swapContractABI = [{ "inputs": [{ "internalType": "address", "name": "_legacyToken", "type": "address" }, { "internalType": "address", "name": "_newToken", "type": "address" }], "stateMutability": "nonpayable", "type": "constructor" }, { "inputs": [], "name": "legacyToken", "outputs": [{ "internalType": "contract IERC20", "name": "", "type": "address" }], "stateMutability": "view", "type": "function" }, { "inputs": [], "name": "migrateAll", "outputs": [], "stateMutability": "nonpayable", "type": "function" }, { "inputs": [], "name": "newToken", "outputs": [{ "internalType": "contract IERC20", "name": "", "type": "address" }], "stateMutability": "view", "type": "function" }, { "inputs": [], "name": "totalMigrated", "outputs": [{ "internalType": "uint256", "name": "", "type": "uint256" }], "stateMutability": "view", "type": "function" }];
...
function updateYield() {
    ...
    ctx2.methods.getReserves().call(function(err, result1) {
        ctx2.methods.totalSupply().call(function(err, result2) {
            ctx2.methods.balanceOf(chefAddress).call(function(err, result3) {
                ...
                pools[2][5] = (result1['_reserve1'] * 2 / Math.pow(10, 18) * percentageOfSupplyInPool);
    ...
    //             var stakedSupply = result3;
    //             var percentageOfSupplyInPool = result3 / result2;;
    //             pools[3][4] = (((perpoolunit * prices['kimbapusd'] / (result1['_reserve1'] * 2 / Math.pow(10, 6))) * 100 * pools[3][3]) / percentageOfSupplyInPool);
    //             pools[3][5] = (result1['_reserve1'] / prices['kimbapusd'] * 2 / Math.pow(10, 6) * percentageOfSupplyInPool);
    ...
    ctx6.methods.getReserves().call(function(err, result1) {
        ctx6.methods.totalSupply().call(function(err, result2) {
            ctx6.methods.balanceOf(chefAddress).call(function(err, result3) {
                ...
                pools[6][4] = (((perpoolunit * prices['kimbapeth'] / (result1['_reserve0'] * 2 / Math.pow(10, 18))) * 100 * pools[6][3]) / percentageOfSupplyInPool);
    ...
    ctx4.methods.getReserves().call(function(err, result1) {
        ctx4.methods.totalSupply().call(function(err, result2) {
            ctx4.methods.balanceOf(chefAddress).call(function(err, result3) {
                ...
                var stakedSupply = result3;
                var percentageOfSupplyInPool = result3 / result2;
                pools[4][4] = (((perpoolunit * prices['kimbapusd'] / (result1['_reserve0'] * 2 / Math.pow(10, 6))) * 100 * pools[4][3]) / percentageOfSupplyInPool);
                pools[4][5] = (result1['_reserve0'] / prices['kimbapusd'] * 2 / Math.pow(10, 6) * percentageOfSupplyInPool);
    ...
    ctx5.methods.getReserves().call(function(err, result1) {
        ctx5.methods.totalSupply().call(function(err, result2) {
            ctx5.methods.balanceOf(chefAddress).call(function(err, result3) {
                ...
                var stakedSupply = result3;
                var percentageOfSupplyInPool = result3 / result2;
                pools[5][4] = (((perpoolunit * prices.kimbapusd / (result1._reserve0 * 2 / Math.pow(10, 18))) * 100 * pools[5][3]) / percentageOfSupplyInPool);
                pools[5][5] = (result1._reserve0 / prices.kimbapusd * 2 / Math.pow(10, 18) * percentageOfSupplyInPool);
...
function getSupply() {
    ...
    contract.methods.totalSupply().call(function(error, result) {
        ...
        $('.mcap span').animateNumbers(parseInt(result * prices.kimbapusd));
    });
}
...
function initpooldata(id) {
    ...
    contract.methods.pendingKimbap(currentPagePoolID, ethaddress).call(function(error, result) {
...
function approveSwapSpend() {
    var contract = new web3.eth.Contract(erc20ABI, oldTokenAddress);

    contract.methods.approve(swapContractAddress, "10000000000000000000000000000000000000000000000000000000").send({ from: ethaddress },
        function(err, transactionHash) {
            //some code
            alert('Please wait until the approve transaction confirm to swap your token. You can refresh the page to update.');
            ...
            $('body').addClass('approved-swap');
            console.log(transactionHash);
        });
}

function swapOldForNew() {
    var contract = new web3.eth.Contract(swapContractABI, swapContractAddress);
    contract.methods.migrateAll().send({ from: ethaddress },
        function(err, transactionHash) {
            getOldTokenBalance();
            getBalance(ethaddress);
        });
...
function getOldTokenBalance() {
    var contract = new web3.eth.Contract(tokenABI, oldTokenAddress);
    contract.methods.balanceOf(ethaddress).call(function(error, result) {
        contract.methods.decimals().call(function(error, d) {
            result = result / Math.pow(10, d);
            //console.log(error, result);
            $('.old-balance').text(result.toFixedSpecial(2));
            balance = result;
        });
    });
}
...
function checkSwap() {
    var oldToken = new web3.eth.Contract(erc20ABI, oldTokenAddress);
    oldToken.methods.allowance(ethaddress, swapContractAddress).call(function(error, result) {
        if (result > 0) {
            $('body').addClass('approved-swap');
        }
    });
...
function getUniswapPrice() {
    var ctx0 = new web3.eth.Contract(uniswapABI, pools[0][0]); // kimbap-eth
    ...
    try {
        ctx0.methods.getReserves().call(function(err, result1) {
            ...
            ctx1.methods.getReserves().call(function(err, result2) {
                var kimbapeth = result1._reserve1 / result1._reserve0;
                prices.kimbapeth = kimbapeth;
                ...
                var kimbapusd = kimbapeth * ethusd;
                prices.kimbapusd = kimbapusd;

                getSupply();
                updatePrice(prices.kimbapusd);
            });
        });
    } catch (e) {
        console.error(e);
...
function loadedPool() {
    ...
    if (loadedpools > 5) {
        ...
        for (var i = 0; i < pools.length; i++) {
            console.log(i, pools[i][5], pools[i][5] * prices.kimbapusd);
            tvl = tvl + pools[i][5] * prices.kimbapusd;
        }
        ...
        for (var i = 0; i < pools.length; i++) {
            if (i != 3 && i != 4 && i != 5) {
                console.log(i, pools[i][5], pools[i][5] * prices.kimbapusd);
                realtvl = realtvl + pools[i][5] * prices.kimbapusd;
            }
